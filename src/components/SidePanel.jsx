      ...
      <div style={{
        ...
        <div style={styles.menu}>
          ...
          <MenuItem icon="book" label="Syllabus" theme={theme}
            onClick={() => { playClick(); onMenuItem('syllabus'); onClose(); }} />

          <MenuItem icon="wind" label="Breathing" theme={theme} badge="Soon"
            onClick={() => { playClick(); onMenuItem('breathing'); onClose(); }} />
...
function MenuItem({ icon, label, theme, badge, danger, onClick }) {
  return (
    <button
      style={{ ...styles.menuItem, color: danger ? '#ef4444' : theme.text }}
      onClick={onClick}
    >
      ...
      {badge && (
        <span style={{ ...styles.badge, background: theme.primary }}>{badge}</span>
      )}
    </button>
  );
}
  ...
  return (
    <div style={styles.toggleItem}>
      ...
      <button
        style={{
          ...styles.toggleTrack,
          background: value ? theme.primary : 'rgba(255,255,255,0.1)',
        }}
...
const styles = {
  backdrop: {
    ...
    zIndex: 40, transition: 'opacity 0.22s ease', backdropFilter: 'blur(2px)',
  },
  drawer: {
    position: 'fixed', top: 0, left: 0, bottom: 0, width: 268, zIndex: 50,
  ...
  menuItem: {
    ...
    cursor: 'pointer', fontSize: 14, textAlign: 'left', transition: 'background 0.12s',
  },
  ...
  toggleTrack: {
    position: 'relative', width: 32, height: 18, border: 'none', cursor: 'pointer',
    borderRadius: 9, transition: 'background 0.2s', flexShrink: 0,
  ...
  footer: {
    padding: '12px 20px', display: 'flex', alignItems: 'center', gap: 4,
  },
